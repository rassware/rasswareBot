...
#TIPP: Put mysql credetials into ~/.my.cnf
...
/usr/bin/mysql -D climadb -e'UPDATE sensors SET temperature_C_dec = temperature_C WHERE temperature_C <> "" AND temperature_C_dec IS NULL;'
/usr/bin/mysql -D climadb -e'UPDATE sensors SET humidity_dec = humidity WHERE humidity <> "" AND humidity_dec IS NULL;'
